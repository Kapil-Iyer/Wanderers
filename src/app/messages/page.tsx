  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: "var(--color-bg)" }}>
      ...
      <header className="sticky top-0 z-40"
        style={{ background: "rgba(9,9,11,0.85)", borderBottom: "1px solid rgba(249,115,22,0.08)", backdropFilter: "blur(16px)" }}>
          ...
          {conversations.length > 0 ? (
                    ...
                    >
                      <div className="relative w-12 h-12 rounded-full flex items-center justify-center text-lg shrink-0"
                        ...
                        {convo.avatar.length <= 2
                          ? <span className="text-xs font-bold" style={{ color: "#F97316" }}>{convo.avatar}</span>
                          : convo.avatar}
                        {convo.unread > 0 && (
                      ...
                      {convo.unread > 0 && (
                        <div className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                          style={{ background: "linear-gradient(135deg, #F97316, #FBBF24)", boxShadow: "0 0 10px rgba(249,115,22,0.4)" }}>
                          <span className="text-[10px] font-bold" style={{ color: "#1a0a00" }}>{convo.unread}</span>
                        </div>
                      )}
                    </motion.button>
                  </StaggerItem>
            ...
            <Reveal className="mb-10">
              <div className="relative overflow-hidden rounded-3xl p-8 text-center"
                style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(249,115,22,0.12)" }}>
                {/* warm pool */}
                <div className="absolute inset-x-0 bottom-0 h-24 pointer-events-none"
                  style={{ background: "radial-gradient(ellipse 60% 100% at 50% 100%, rgba(249,115,22,0.14), transparent 70%)" }} />
                <div className="relative">
                  <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4"
                    style={{ background: "rgba(249,115,22,0.1)", border: "1px solid rgba(249,115,22,0.2)" }}>
                    <MessageCircle className="w-7 h-7" style={{ color: "#F97316" }} />
                  </div>
                  ...
                  <Link href="/map">
                    <motion.div
                      ...
                      style={{ background: "linear-gradient(135deg, #F97316, #FBBF24)", color: "#1a0a00", boxShadow: "0 0 20px rgba(249,115,22,0.3)" }}
                  ...
                  <StaggerItem key={friend.id}>
                    <div className="flex items-center gap-3.5 p-3 rounded-2xl"
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {friend.currentEvent ? (
                          <p className="text-[11px] truncate" style={{ color: "#F97316" }}>Live · {friend.currentEvent}</p>
                        ) : (
          ...
          <Reveal delay={0.1}>
            <Link href="/home">
              <motion.div
                ...
                style={{ background: "rgba(249,115,22,0.06)", border: "1px dashed rgba(249,115,22,0.3)" }}
                whileHover={{ scale: 1.01, background: "rgba(249,115,22,0.1)" }}
                transition={{ type: "spring", stiffness: 300, damping: 24 }}>
                <Sparkles className="w-5 h-5 shrink-0" style={{ color: "#F97316" }} />
